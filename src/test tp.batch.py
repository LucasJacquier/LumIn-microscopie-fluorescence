...
def image_peak_save_show(
    ...
    array_spot_R = tp.batch(
        film_one_color,
        diameter=batch_parameters[0],
        minmass=batch_parameters[1],
        percentile=batch_parameters[2],
    )  # Peak extraction
    ...
    int_coordinate = [
        (int(ordinate_image_spot[i]), int(abscissa_image_spot[i]))
        for i in range(length)
    ]  # Establishment of integers peak coordinates

    modified_image = Image.new(
        "RGB", (film_shape[1], film_shape[2])
    ...
    for x in range(film_shape[1]):
        for y in range(film_shape[2]):
            ...
            origin_image.putpixel(
                (x, y), (color, color, color)
            )  # Painting of the image with the red dots

    name_modif = str(
        path[re.search("Films/", path).span()[1] : -4]
        + "_"
        + str(batch_parameters)
        + "_R0_detectedpeaks"
    )  # Name of the new file

    modified_image.save(
        r"C:\Users\lucas\Documents\sciences\Mes Recherches\2022_09 ARIA 1er Stage\Images\img_auto(2022_10_21)\ "
        + name_modif
        + ".png"
...
def image_peak_save_show_complete(
    path: Union[Path, str],
    color_sequence: List[str],
    frame_selection: int,
    batch_parameters: list,
    ...
    film_rouge, film_vert = (
        ...
        load_data(path, color_sequence)[color_sequence[1]],
    )  # chargement du film
    ...
    modified_image = Image.new(
        "RGB", (film_shape[1], film_shape[2])
    )  # Creation of the red dots image
    origin_image = Image.new(
        "RGB", (film_shape[1], film_shape[2])
    ...
    modified_image.save(
        r"C:\Users\lucas\Documents\sciences\Mes Recherches\2022_09 ARIA 1er Stage\Images\img_auto\ "
        + name_modif
        + ".png"
    )
    origin_image.save(
        ...
        + name_origin
        + ".png"
    )  # Saving of the images with the specified path
